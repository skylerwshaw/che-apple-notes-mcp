    ...
    func listNotes(options: NoteListOptions = NoteListOptions()) throws -> [Note] {
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            var note = Note(
                ...
                storeUUID: storeUUID,
                creationDate: SQLQueries.coreDataDate(columnDoubleOptional(stmt, 6)),
                modificationDate: SQLQueries.coreDataDate(columnDoubleOptional(stmt, 7)),
                isPinned: sqlite3_column_int(stmt, 8) != 0,
                isPasswordProtected: sqlite3_column_int(stmt, 9) != 0,
                snippet: columnText(stmt, 10),
                shared: sqlite3_column_int(stmt, 11) != 0,
                bodyText: nil,
                bodyHTML: nil
            )
